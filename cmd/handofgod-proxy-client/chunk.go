...
// safely passed to Session.Write for a given zone and label-entropy mode.
// Writes exceeding this size are silently dropped by dns.Client's handleSlot
// when EncodeFQDNMode returns ErrPayloadTooLarge (see dns/client.go:
...
//
// This function is duplicated (verbatim) in cmd/handofgod-proxy-server/chunk.go
...
// of datagramBytes. Returns 0 if the codec would produce empty output.
